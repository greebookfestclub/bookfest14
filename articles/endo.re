...
== サーバーAPIについて
サーバー向けのGoogle Play Developer APIについては、大きく分けて定期課金プラン管理用のものと、ユーザーの定期課金情報を取得するものがあります。今回ここで検討するのはユーザーの定期課金情報を取得する方を解説していきます。具体的には2022年以前のpurchases.subscriptions.get(v1 APIと呼びます)と、2022年5月登場のpurchases.subscriptionsv2.get(v2 APIと呼びます)について見ていきます。
...
これで困ったのでもろもろ確認をしました。最終的には各自問い合わせて頂いた方が良さそうですが、各種資料から読み取った内容などをまとめると以下の内容になります。
...
新しいv2 APIは、APIの値やドキュメントの方は微妙に整備されていないなど、慎重にならざるを得ない部分も残っています。ですがGoogleの新しい定期課金はかなりの柔軟さを持てそうな新しい仕組みで、上手く使えば複雑になりそうな定期課金を管理するために必須になるでしょう。問い合わせなどしなければならないとはいえ今後もウォッチしていきましょう。
